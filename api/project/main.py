...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mssql+pyodbc://tap2023:tap2023@APINP-ELPT8R8U9\SQLEXPRESS/capstoneproject?driver=SQL Server'
db = SQLAlchemy(app)
...
def delete(id):
    # print(id)
    query=f"DELETE FROM Signup WHERE user_id={id}"
    with db.engine.begin() as conn:
        conn.exec_driver_sql(query)
        return redirect(url_for('home'))
...
def edit(id):
    user=Signup.query.filter_by(user_id=id).first()
    if request.method=="POST":
        uName=request.form.get('first_name')
        uDesc=request.form.get('last_name')
        uEmail=request.form.get('email')
        uNumber=request.form.get('phone')

        sql_query=f"UPDATE Signup SET first_name='{uName}',last_name='{uDesc}',email='{uEmail}',phone='{uNumber}' WHERE user_id='{id}'"
